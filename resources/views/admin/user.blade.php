...
<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="flex flex-col sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
        <div>
            <a href="{{ route('admin.create-user') }}" class="inline-flex items-center text-white font-medium rounded-lg text-sm px-3 py-2 bg-blue-600">
                Tambah Data
            </a>
        </div>
        <div class="relative">
        ...
        <tbody>
            ...
            <tr class="bg-white border border-gray-300">
                ...
                <td class="border border-gray-300 px-6 py-4">
                    <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('images/default-avatar.png') }}" alt="Avatar" class="w-12 h-12 rounded-full object-cover">
                </td>
                ...
                <td class="px-6 py-4 flex space-x-3">
                    ...
                    <form action="{{ route('admin.delete-user', $user->id) }}" method="POST" class="inline delete-form" data-title="{{ $user->id }}">
                        @csrf
                        @method('DELETE')
